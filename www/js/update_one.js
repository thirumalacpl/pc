...
$(document).one('pageshow', '#update_one', function(){  
	...
	$(document).off('click', '#find_update1').on('click', '#find_update1', function() { 
		$.ajax({url: 'http://staging.eimpressive.com/test/pc/slimrestapi-dev-test/searchlpa_part.php',
...
function searchlpa(part){
	$.ajax({url: 'http://staging.eimpressive.com/test/pc/slimrestapi-dev-test/searchlpa.php?find_part='+part,
		data:$('#lpa_update_form').serialize(),
		type: 'post',                   
		async: 'true',
		crossDomain: true,
